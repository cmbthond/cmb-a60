...
document.getElementById('openDisplay').addEventListener('click',()=>{ displayWindow=window.open('display.html','cmb-display','noopener=false'); if(!displayWindow) alert('Trình duyệt đang chặn cửa sổ màn hình lớn. Hãy cho phép pop-up và thử lại.'); });
let countdownTimer, showcaseTimer;
function startCountdown(){clearInterval(countdownTimer); countdownTimer=setInterval(()=>{if(appState.state!==2){clearInterval(countdownTimer);return}if(appState.count>0){appState.count--;render();if(appState.count===0){clearInterval(countdownTimer);setTimeout(()=>selectState(3),1300)}}},1000)}
...
channel.onmessage = e => { if(e.data?.type === 'request-state') render(); };
render();
